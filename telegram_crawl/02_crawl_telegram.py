...
messages = pd.read_csv("data/telegram_message.csv",
...
forward_channel_df = messages[["for_channel_id", "for_channel_title", "for_channel_username", "for_channel_title"]]
...
async def main():
    ...
    current_id = 527
    for channel_username in forward_channel_df["for_channel_username"][current_id:600]:
        print(f"{current_id}: {channel_username}")
        current_id += 1
        try:
        ...
        except (UsernameNotOccupiedError, ValueError) as f:
            print(f)
            continue
